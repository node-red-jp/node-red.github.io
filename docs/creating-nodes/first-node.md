...
このフレームワークを使うことで、テストフローを作成することができ、さらにノードの設定値と出力結果が期待通りに動作していることを確認できます。例えば、lower-caseノードにユニットテストを追加するため、`_spec.js`ファイルを含んだ`test`フォルダを、自身のノードモジュールパッケージに追加しましょう。
